...
function AdminProductos() {
  ...
  const [form, setForm] = useState({ nombre: '', descripcion: '', marca: '', precio: '', stock: '', categoriaId: '' })
        ...
        <form onSubmit={handleCrearProducto} className="bg-white border border-gray-200 rounded-lg p-4 mb-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <input type="text" placeholder="Marca" value={form.marca}
              ...
              className="border border-gray-300 rounded-md px-3 py-2 text-sm" required />
            <input type="number" placeholder="Precio S/" value={form.precio}
            ...
            <select value={form.categoriaId} onChange={(e) => setForm({ ...form, categoriaId: e.target.value })}
              className="border border-gray-300 rounded-md px-3 py-2 text-sm bg-white" required>
              <option value="">Selecciona categoria</option>
              {categorias.map((c) => <option key={c.id} value={c.id}>{c.nombre}</option>)}
            </select>
            <input type="text" placeholder="Descripcion (opcional)" value={form.descripcion}
              ...
              <div>
                {editandoStock === producto.id ? (
                  <div className="flex gap-1 items-center">
                    ...
                    <button onClick={() => handleActualizarStock(producto.id)}
                      className="text-xs bg-blue-600 text-white px-2 py-1 rounded-md">OK</button>
                  </div>
                ) : (
                  <button onClick={() => { setEditandoStock(producto.id); setNuevoStock(producto.stock) }}
