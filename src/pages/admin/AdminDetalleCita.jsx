...
function AdminDetalleCita() {
  ...
  return (
    <LayoutAdmin>
      <Link to="/admin/citas" className="text-sm text-blue-600 hover:underline mb-4 inline-block">
        ← Volver a citas
      </Link>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <select value={cita.estado} onChange={(e) => cambiarEstado(e.target.value)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="flex flex-col gap-4">
          ...
          <form onSubmit={handleGuardarCotizacion} className="bg-white border border-gray-200 rounded-lg p-4">
            ...
            <textarea placeholder="Descripcion del trabajo" value={descCotizacion}
              onChange={(e) => setDescCotizacion(e.target.value)}
              rows="3" className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm mb-2" />
            <div className="flex gap-2 items-center">
              <input type="number" placeholder="Monto S/" value={montoCotizacion}
                onChange={(e) => setMontoCotizacion(e.target.value)}
                className="border border-gray-300 rounded-md px-3 py-2 text-sm w-36" />
              <button type="submit" disabled={guardandoCotizacion}
                className="bg-blue-600 text-white text-sm px-4 py-2 rounded-md hover:bg-blue-700 disabled:opacity-50">
                {guardandoCotizacion ? 'Guardando...' : 'Enviar cotizacion'}
              </button>
            </div>
            {cotizacion && <p className="text-xs text-gray-500 mt-2">Estado: {cotizacion.estado}</p>}
          </form>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          ...
          {cotizacion ? (
            <>
              ...
              <button onClick={handleRegistrarPago} disabled={registrandoPago}
...
export default AdminDetalleCita